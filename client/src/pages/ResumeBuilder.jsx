...
const ResumeBuilder = () => {
  ...
  // resumeId comes from route param /app/builder/:resumeId
  const { resumeId } = useParams()
  const { token } = useSelector(state => state.auth)
  ...
  const saveResume = async () => {
    try {
      ...
      // Remove img object from updated resume data (will be sent separately)
      if (typeof resumeData.personal_info.image === 'object') {
        delete updatedResumeData.personal_info.image
      }
          ...
          <div className='relative lg:col-span-5 rounded-lg overflow-hidden'>
            <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1'>
              ...
              <div className='flex justify-between items-center mb-6 border-b border-gray-300 py-1'>
                ...
                <div className='flex items-center'>
                  {activeSectionIndex !== 0 && (
                    <button onClick={() => setActiveSectionIndex(prev => Math.max(prev - 1, 0))} className='flex items-center gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all'>
                      <ChevronLeft className='size-4' /> Previous
                    </button>
                  )}
                  <button
